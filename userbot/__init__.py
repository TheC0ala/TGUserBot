...
if version_info[0] < 3 or version_info[1] < 6:
    LOGS.info("En az python 3.6 versionuna sahib olmalısınız."
              "Birden çox özellik buna bağlıdır. Bot söndürülür.")
    quit(1)

# TGUSERBOT
# TGUSERBOT
CONFIG_CHECK = os.environ.get(
    "___________XAIŞ_______BU_____SETRI_____SILIN__________", None)

if CONFIG_CHECK:
    LOGS.info(
        "Zehmet birinci hashtag'da seçilen setri config.env qovluqundan silin"
    )
    quit(1)
...
if not LANGUAGE in ["EN", "TR", "AZ", "UZ", "DEFAULT"]:
    LOGS.info("Namelum dil yazdınız. Buna göre DEFAULT işledilir.")
    LANGUAGE = "DEFAULT"
    
# VERSİON
TGUSERBOT_VERSION = "v1.0"

# TELEGRAM API KEY, API HASH
...
async def check_botlog_chatid():
    if not BOTLOG_CHATID and LOGSPAMMER:
        LOGS.info(
            "Xüsusi xeta gündeliyinin işlemesi üçün BOTLOG_CHATID olmalıdır.")
        quit(1)

    elif not BOTLOG_CHATID and BOTLOG:
        LOGS.info(
            "Gündeliye qeyd etme xüsusiyyetinin işlemesi üçün BOTLOG_CHATID olmalıdır.")
        quit(1)
...
Hesabınızı bot'a çevire bilersiz ve bunları işlede bilersiz. Unutmayın, siz başqasının botunu idare etmessiniz! Altdakı GitHub adresinde bütün qurulum detalları var.""",
                    buttons=[
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ Hey! menim mesajlarımı düzeltmeye çalışma! Özünə bir @UserBotTG qur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"**TGᑌSEᖇᗷOT Əla İşləyir⚡** [TGUserBot](https://t.me/userbottg) __İşləyir...__\n\n**Yüklənən Modul Sayı:** `{len(CMD_HELP)}`\n**Sahife:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
        ...
        async def bilgi(event):
            ...
            try:
                ...
                return await event.answer("❌ Bu modula açıqlana yazılmayıb.", cache_time=0, alert=True)
        ...
        LOGS.info(
            ...
            "Aktivlesdirmek üçün bir token yazın ve inline desteyini açın. "
            "Bunnan başqa probleminiz varsa Support qrupuna yaza bilersiz."
        )
    ...
    try:
        ...
        LOGS.info(
            ...
            "Ortam deyiskenlerinizi / config.env faylında yoxlayın."
        )
        quit(1)
